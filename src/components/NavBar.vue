<template>
  <nav class="flex justify-center fixed top-0 bg-violet-500 h-12 w-screen z-10">
    <div class="flex justify-between items-center w-screen sm:w-4/5 max-w-screen-lg">
      <RouterLink to="/" class="font-logo font-semibold pl-2 italic text-2xl text-white">BETMATES</RouterLink>
      <ul class="flex justify-center items-center h-full">
        ...
        <li v-if="!authStore.isLoggedIn" class="flex border-x-[1px] px-2 border-neutral-600 shadow-inner text-white text-sm w-full h-full">
          <RouterLink class="flex items-center" to="/register">Register</RouterLink>
        </li>
        <li
          v-if="betStore.accountBalance"
          @click="betStore.addMoney(5)"
        ...
        >
          ...
          <p class="text-nowrap px-1">Hi {{ authStore.user?.displayName }}</p>
        </li>
